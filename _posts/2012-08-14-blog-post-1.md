...
\
Combining the results **(R1)** and
**(R2)**, we prove the theorem.\
**Note:** We assumed the existence of an orthonormal basis for finite
...
the origin in the direction of $w$, a 1-D subspace, is given as
...
The vector $v1$ is given as $v1=ww^{\top}v$. The magnitude of the
...
have n-dimensional feature vectors, and we want to project them onto a
line through the origin. We are trying to find a best-fit line to
represent all sample data points. Let the line be in the direction of a
unit vector $w$.\
...
$z_{i}= \[z_{i1},z_{i2},...z_{ip}\]^{\top}$, where
...
**Note:** For the sample covariance matrix, we replace the covairances with
...
the n-dimensional data points, which are denoted as $x_{i}$.
...
Since we have assumed the data to be centred, sample covariance matrix
...
same as the eigenvalue decomposition.\
...
projections along them (eigenvalues). 
